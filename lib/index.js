...
function innerRpc(request, callback) {
    ...
    var errorMessage = 'NPCcoin JSON-RPC: ';
    ...
    var req = this.protocol.request(options, function (res) {
        ...
        res.on('end', function () {
            ...
            if (res.statusCode === 500 && buf.toString('utf8') === 'Work queue depth exceeded') {
                var exceededError = new Error('NPCcoin JSON-RPC: ' + buf.toString('utf8'));
...
// For definitions of RPC calls, see various files in: https://github.com/npccoin/npccoin/tree/master/src
RpcClient.callspec = {
